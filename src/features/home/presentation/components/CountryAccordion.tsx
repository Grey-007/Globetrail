...
export const CountryAccordion: React.FC<{
  ...
  return (
    <div className="border border-fine-border rounded-2xl bg-card-surface overflow-hidden mb-4 group/accordion">
      <div className="flex items-center justify-between p-4 relative">
        ...
        <div className="flex items-center gap-3 z-10 pointer-events-none">
          ...
          <span className="font-semibold text-lg text-white tracking-wide">{country.name}</span>
        </div>
        <div className="flex items-center gap-3 z-10">
          ...
          <span className="text-textMuted font-mono text-sm pointer-events-none">
            {country.places.length} {country.places.length === 1 ? 'place' : 'places'}
          </span>
          <button
            onClick={() => onAddPlace(country.id)}
            className="p-1 rounded-full bg-white/5 text-white hover:bg-white/10 transition-colors focus:outline-none ml-2"
          ...
          <div className="pointer-events-none ml-1">
            <ChevronDown 
              className={cn(
                "w-5 h-5 text-textMuted transition-transform duration-300", 
          ...
          >
            <div className="p-4 pt-0 flex flex-col gap-3">
              ...
              {country.places.length === 0 && (
                <div className="text-center py-6 text-sm text-textMuted border border-dashed border-fine-border rounded-xl">
                  No places added yet.
                </div>
              )}
            </div>
